...
export default function VideoEditor() {
  ...
  const state     = location.state as { file?: FileInfo } | null
  const stateFile = state?.file ?? null
  ...
  // Fetch timeline frames whenever file changes
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-3 border-b flex items-center gap-3 shrink-0">
        ...
        {/* Allow changing file only in compress mode */}
        {!isEditorMode && (
          <button
            onClick={() => { setSelectedFile(null); setFrames([]) }}
            className="p-1.5 rounded-md hover:bg-muted text-muted-foreground hover:text-foreground transition-colors shrink-0"
          >
            <X className="w-4 h-4" />
          </button>
      ...
      <div className="flex-1 overflow-y-auto">
        {/* Video player */}
        <div className="relative bg-black aspect-video w-full max-h-44 shrink-0">
          <video
            ref={videoRef}
            src={videoSrc}
        ...
        <div className="p-5 flex flex-col gap-5">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-2 text-xs">
              ...
              <button
                onClick={() => handleTrimChange(Math.max(0, trimStart - 1), trimEnd)}
                className="w-6 h-6 rounded-md bg-muted hover:bg-accent flex items-center justify-center shrink-0"
              >−</button>
              <span className="tabular-nums font-medium bg-muted px-2 py-1 rounded-md text-center w-14 shrink-0">
                {formatTime(trimStart)}
              </span>
              <button
                onClick={() => handleTrimChange(Math.min(trimStart + 1, trimEnd - 0.25), trimEnd)}
                className="w-6 h-6 rounded-md bg-muted hover:bg-accent flex items-center justify-center shrink-0"
              >+</button>

              <button
                onClick={() => handleTrimChange(trimStart, currentTime > trimStart ? currentTime : trimEnd)}
                className="flex-1 px-2 py-1 rounded-md border hover:bg-accent transition-colors text-center"
              >
                Set end to playhead
              </button>

              <button
                onClick={() => handleTrimChange(trimStart, Math.max(trimEnd - 1, trimStart + 0.25))}
                className="w-6 h-6 rounded-md bg-muted hover:bg-accent flex items-center justify-center shrink-0"
              >−</button>
              <span className="tabular-nums font-medium bg-muted px-2 py-1 rounded-md text-center w-14 shrink-0">
                {formatTime(trimEnd)}
              ...
              <button
                onClick={() => handleTrimChange(trimStart, Math.min(trimEnd + 1, duration))}
                className="w-6 h-6 rounded-md bg-muted hover:bg-accent flex items-center justify-center shrink-0"
              >+</button>
